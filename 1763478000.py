...
class ChatClient(tk.Tk):
    ...
    async def connect_and_start(self):
        """Establishes TCP connection and starts the network tasks."""
        ...
        try:
            ...
            self.display_message("Connection Error: Server refused connection. TCP functions disabled.")
            self.after(0, lambda: messagebox.showerror("Error", "Server refused connection. TCP functions disabled."))
        except Exception as e:
            error_message = f"{type(e).__name__}: {e}"
            self.display_message(f"Connection Error: {error_message}. TCP functions disabled.")
            self.after(0, lambda msg=error_message: messagebox.showerror("Error", f"Failed to connect (TCP): {msg}"))
    ...
    async def _send_tcp_message(self, full_message):
        ...
        try:
            if self.tcp_writer:
                ...
                self.tcp_writer.write(data_to_send)
                await self.tcp_writer.drain()
            else:
    ...
    def send_message(self, event=None):
        """Handles sending messages via TCP."""
...
class ApplicationLauncher:
    """Handles initialization and selection between running as ChatClient or ChatServer."""

    def __init__(self):
        self.loop = self._setup_asyncio_loop()
        self.root = tk.Tk()
        self.root.withdraw()
    ...
    async def _server_runner(self):
        """Asynchronously runs the server setup and listening loop."""
        ...
        if server_host:
            server = ChatServer(host=server_host, loop=self.loop)
    ...
    def run_server(self):
        """Starts the server implementation."""
        try:
    ...
    def run_client(self):
        ...
        client_app = ChatClient(loop=self.loop)

        if not client_app.server_ip:
            return

        def start_network_tasks():
            if client_app.running:
                asyncio.run_coroutine_threadsafe(client_app.connect_and_start(), self.loop)
    ...
    def run(self):
        """Prompts for mode selection and executes the chosen mode, handling cleanup."""
        try:
            ...
            print(f"An error occurred during application setup: {e}")
        finally:
            self.cleanup()
    ...
    def cleanup(self):
        ...
        if self.root and self.root.winfo_exists():
            self.root.destroy()
        if self.loop and not self.loop.is_running() and not self.loop.is_closed():
            self.loop.close()
