...
class DungeonItemSearchEventStrategy(EventStrategy):
    def execute(self, event, temp_enemy_id=None):
        if not self.game_manager.current_dungeon_items:
            self.game_manager.message_queue = ["You search around, but find nothing."]
            self.game_manager.set_state(GameState.MESSAGE)
            self.game_manager._pending_next_event_id = self.game_manager.current_dungeon_id + "_explore_choice"
            return
        random_item_id = self.game_manager.current_dungeon_items[
            pyxel.rndi(0, len(self.game_manager.current_dungeon_items) - 1)
        ]
        item_get_event = {
            "id": f"dynamic_item_get_{random_item_id}",
            "type": "item_get",
            "message": event["message"],
            "item_id": random_item_id,
            "quantity": 1,
        }
        self.game_manager._event_strategies[EventType.ITEM_GET].execute(item_get_event)
        self.game_manager._pending_next_event_id = self.game_manager.current_dungeon_id + "_explore_choice"
...
class MessageDrawStrategy(BaseGameDrawStrategy):
    def draw_screen(self):
        ...
        for msg in message_queue:
            display_lines.extend(self.app_instance._wrap_text(msg, main_text_max_chars))
        current_y = y_offset
            ...
            if not player_stats["inventory"]:
                ...
                for item_id, quantity in player_stats["inventory"].items():
                    ...
                    if item:
                        ...
                        wrapped_item_lines = self.app_instance._wrap_text(item_display_text, indented_text_max_chars)
...
class ChoiceDrawStrategy(BaseGameDrawStrategy):
    def draw_screen(self):
        ...
        indented_text_max_chars = 48
        current_y = y_offset
        event_message_lines = self.app_instance._wrap_text(current_event["message"], main_text_max_chars)
        ...
        for i, choice in enumerate(current_event["choices"]):
            ...
            wrapped_choice_lines = self.app_instance._wrap_text(display_choice_text, choice_text_max_chars)
...
class BattleDrawStrategy(BaseGameDrawStrategy):
    def draw_screen(self):
        ...
        for entry in battle_log:
            all_wrapped_log_lines.extend(self.app_instance._wrap_text(entry, main_text_max_chars))
...
class GameState(Enum):
    ...
    ITEM_GET = (auto(), ItemGetStateStrategy, MessageDrawStrategy)
    STATUS = (auto(), StatusStateStrategy, MessageDrawStrategy)
...
class App:
    ...
    def _load_game_data_from_file(self):
        try:
            with open("1755961220.json", "r", encoding="utf-8") as f:
                return json.load(f)
        except FileNotFoundError:
            print("Error: 1755961220.json not found.")
            pyxel.quit()
        except json.JSONDecodeError:
            print("Error: Failed to decode 1755961220.json. Check JSON format.")
            pyxel.quit()

    def _load_and_display_rules(self):
        try:
            with open("1755961240.txt", "r", encoding="utf-8") as f:
                rules = f.read()
                print("--- How to Play ---")
                print(rules)
                print("-------------------")
        except FileNotFoundError:
            print("Error: 1755961240.txt (rules) not found.")
